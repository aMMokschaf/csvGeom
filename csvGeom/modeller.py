...
class Modeller:
    ...
    def create_features(row_lists, selected_geometry_type):
        ...
        for row_list in row_lists:
            ...
            if selected_geometry_type == GeoJsonType.POINT and len(row_list[0]) >= 2:

                geometry_type = Modeller.switch_to_multi_geometry(selected_geometry_type)

                transformed_list = Modeller.transform_row_list_for_multi_point(row_list)
                ...
                feature = Modeller.create_feature(row_list, geometry_type)
    ...
    def create_geometry(row_list, selected_geometry_type):
        ...
        if selected_geometry_type == GeoJsonType.POINT:
            return Modeller.create_point_geometry(row_list)

        if selected_geometry_type == GeoJsonType.LINESTRING:
            return Modeller.create_line_string_geometry(row_list)

        if selected_geometry_type == GeoJsonType.POLYGON:
            return Modeller.create_polygon_geometry(row_list)

        if selected_geometry_type == GeoJsonType.MULTI_POINT:
            return Modeller.create_multi_point_geometry(row_list)

        if selected_geometry_type == GeoJsonType.MULTI_LINESTRING:
            return Modeller.create_multi_linestring_geometry(row_list)

        if selected_geometry_type == GeoJsonType.MULTI_POLYGON:
    ...
    def switch_to_multi_geometry(selected_geometry_type):
        ...
        elif selected_geometry_type == GeoJsonType.LINESTRING:
            return GeoJsonType.MULTI_LINESTRING

        elif selected_geometry_type == GeoJsonType.POINT:
